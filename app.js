  ...
  retrieveBtn.addEventListener('click', () => {
    ...
    linkRef.once('value', (snapshot) => {
        if (snapshot.exists()) {
            ...
            // 1. Деактивуємо поле вводу та кнопку
            codeInput.disabled = true;
            retrieveBtn.disabled = true;

            // 2. Змінюємо вигляд кнопки, щоб показати успіх
            retrieveBtn.innerHTML = `<i class="fa-solid fa-check"></i> Посилання готове`;

            // 3. Відображаємо посилання та кнопку "Ввести інший код"
            linkDisplayArea.innerHTML = `
                <a href="${data.url}" target="_blank" rel="noopener noreferrer">Натисніть, щоб перейти <i class="fa-solid fa-external-link-alt"></i></a>
                <button class="reset-btn" id="reset-btn">Ввести інший код</button>
            `;

            // 4. Додаємо обробник для нової кнопки "Ввести інший код"
            document.getElementById('reset-btn').addEventListener('click', () => {
                // Повертаємо все до початкового стану
                codeInput.disabled = false;
                codeInput.value = '';
                retrieveBtn.disabled = false;
                retrieveBtn.innerHTML = `<i class="fa-solid fa-arrow-right-to-bracket"></i> Перейти`;
                linkDisplayArea.innerHTML = '';
                codeInput.focus(); // Фокусуємось на полі вводу
            });
